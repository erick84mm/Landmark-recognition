...
names = [os.path.splitext(os.path.basename(x))[0] for x in val_list]
...
for i,name in enumerate(names):
    if(i % 1000 ==0):
        print("Processed %d, %f " % (i, (start-time.time())))
    img = image.load_img(val_dir + name + ".jpg", target_size=(img_width, img_height))
    x = image.img_to_array(img)
    x /= 255
    x = np.expand_dims(x, axis=0)
    predictions = ResNet50_test.predict_on_batch(x)
    y_classes = predictions.argmax(axis=-1)
    ind = np.argmax(predictions[0])

    
    imgs_names.remove(name)
    file.write(name + "," + str(y_classes[0]) + ' ' + "{0:.2f}".format(predictions[0][ind]) + "\n")
